...
public partial class Age_disorder : BasePage
{
    ...
    protected void AGE_GRP_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        ...
        if (e.CommandName == "delete")
        {
            ...
            Label AGDD_ID = (Label)AGE_GRP.Rows[index].FindControl("AGDD_ID"); // testing for deleting the agdd_id
            foreach (DataRow drow in dt.Rows)
            {
            ...
            if (AGDD_ID.Text != null)
            {
                Hashtable vHashtable = new Hashtable();
                vHashtable.Add("AGDD_ID", AGDD_ID.Text);
                DBManager.ExecInsUps(vHashtable, "DELETE_AGDIS", vATSession);
    ...
    protected void AGE_GRP_RowUpdating(object sender, GridViewUpdateEventArgs e)
    {
        Label DOBS_ID = (Label)AGE_GRP.Rows[e.RowIndex].FindControl("DOBS_ID");
        Label AGDD_ID = (Label)AGE_GRP.Rows[e.RowIndex].FindControl("AGDD_ID");
        TextBox DOBS_DESC = (TextBox)AGE_GRP.Rows[e.RowIndex].FindControl("DOBS_DESC");
        Hashtable vHashtable = new Hashtable();
        vHashtable.Add("DOBS_ID", DOBS_ID.Text);
        vHashtable.Add("DOBS_DESC", DOBS_DESC.Text);
        DBManager.ExecInsUps(vHashtable, "UPDATE_OBSERVATION", vATSession);
        AGE_GRP.EditIndex = -1;
        ShowMsg("Updated Successfully..........");
        BindGrid1();
        DOBS_DESC.Enabled = false;
       
    }

    protected void AGE_GRP_RowEditing(object sender, GridViewEditEventArgs e)
    {
        AGE_GRP.EditIndex = e.NewEditIndex;
        BindGrid1();
        //remove delete button
        foreach (GridViewRow gdr in AGE_GRP.Rows)
        {
            gdr.Cells[9].Visible = false;
        }
    }

    protected void AGE_GRP_RowCancelingEdit(object sender, GridViewCancelEditEventArgs e)
    {
        AGE_GRP.EditIndex = -1;
        BindGrid1();
    }
    ...
    protected void BindGrid1()
    {
        ...
        Hashtable vHashtable1 = new Hashtable();
        vHashtable1.Add("ID", vID);
        DataTable dt = DBManager.Get(vHashtable1, "GET_AGDIS_DETAIL");
        AGE_GRP.DataSource = dt;
        AGE_GRP.DataBind();
        ...
        if (dt.Columns.Count > 4)
        {
            ...
            {
                for (int j = 0; j < AGE_GRP.Rows.Count; j++)
                {
                    string prod_nam1 = dt.Rows[j][0].ToString();
                    if (prod_nam1 != null)
                    {
                        TextBox DOBS_DESC = (TextBox)AGE_GRP.Rows[j].Cells[4].FindControl("DOBS_DESC");
                        
                        DOBS_DESC.Text = dt.Rows[j][2].ToString();
    ...
    //for editing updating and cancel the row End
