...
public partial class INTELLECTUAL_RATING : BasePage
{
            ...
            try
            {
                ...
                if (vATSession.UserType == "ADMIN")
                {
                    if (vID2 != null)
                    {
                        ...
                        DataTable dt1 = DBManager.Get(vHashtable2, "GET_PT_INTELLECTUAL");
                        DataRow vDR1 = RetDR(DBManager.Get(vHashtable2, "GET_PT_INTELLECTUAL"));
                        if (dt1.Columns.Count > 4)
                        {
                            ...
                            if (vDR1 != null)
                            {
                                Textarea2.InnerText = vDR1["PTINT_OBSERVATION"].ToString();
                            }
                            ...
                            for (int j = 0; j < GridView1.Rows.Count; j++)
                            {
                                ...
                                if (vDR1["PTINT_STATUS"].ToString() == "1")
                                {
    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
        if (GridView1.Rows.Count > 0)
        {
            ...
            {
                Label INTE_ID = (Label)grd.FindControl("INTE_ID");
                Label INTE_NAME = (Label)grd.FindControl("INTE_NAME");
                Label INTE_TRAIT = (Label)grd.FindControl("INTE_TRAIT");
                CheckBox checkbox = (grd.Cells[0].FindControl("CheckBox2") as CheckBox);
               
                if (checkbox.Checked)
                {
                    Hashtable vHashtable = new Hashtable();
                    vHashtable.Add("PTINT_ID", TXTID.Value);
                    vHashtable.Add("PTINT_PTPID", vID2);
                    vHashtable.Add("PTINT_INTEID", INTE_ID.Text);
                    vHashtable.Add("PTINT_INTE_TRAIT", INTE_TRAIT.Text);
                    vHashtable.Add("PTINT_OBSERVATION", Textarea2.InnerText);
                    vHashtable.Add("PTINT_STATUS", "1");
                    vHashtable.Add("TYPE", "INS");
                    DBManager.ExecInsUps(vHashtable, "INS_PT_INTELLECTUAL", vATSession);
                }
            }
