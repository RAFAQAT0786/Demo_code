...
test('by year, v1 (ctrl + left/right arrows)', function () {
    ...
    datesEqual(this.dp.viewDate, UTCDate(2015, 2, 31));
    datesEqual(this.dp.date, UTCDate(2015, 2, 31));
    target = this.picker.find('.datetimepicker-days thead th.switch');
...
test('by year, from leap day', function () {
    ...
    // Both updated on keyboard navigation; graceful month-end
    datesEqual(this.dp.viewDate, UTCDate(2015, 1, 28));
    datesEqual(this.dp.date, UTCDate(2015, 1, 28));
    target = this.picker.find('.datetimepicker-days thead th.switch');
    equal(target.text(), 'February 2015', 'Title is "February 2015"');
});
