...
public partial class CEREBAL_PALSY_RATING : BasePage
{
            ...
            try
            {
                ...
                if (vATSession.UserType == "ADMIN")
                {
                    if (vID2 != null)
                    {
                        ...
                        DataTable dt1 = DBManager.Get(vHashtable2, "GET_PT_CEREBAL_PALSY_NEW");
                        DataRow vDR1 = RetDR(DBManager.Get(vHashtable2, "GET_PT_CEREBAL_PALSY_NEW"));
                        if (dt1.Columns.Count > 4)
                        {
                            ...
                            if (vDR1 != null)
                            {
                                Textarea2.InnerText = vDR1["PTCP_OBSERVATION"].ToString();
                            }
                            ...
                            for (int j = 0; j < GridView1.Rows.Count; j++)
                            {
                                ...
                                if (vDR1["PTCP_STATUS"].ToString() == "1")
                                {
    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
        ...
        if (GridView1.Rows.Count > 0)
        {
            ...
            {
                Label CEREBAL_ID = (Label)grd.FindControl("CEREBAL_ID");
                Label CEREBAL_NAME = (Label)grd.FindControl("CEREBAL_NAME");
                Label CEREBAL_TRAIT = (Label)grd.FindControl("CEREBAL_TRAIT");
                CheckBox checkbox = (grd.Cells[0].FindControl("CheckBox2") as CheckBox);

                if (checkbox.Checked)
                {
                    Hashtable vHashtable = new Hashtable();
                    vHashtable.Add("PTCP_ID", TXTID.Value);
                    vHashtable.Add("PTCP_PTPID", vID2);
                    vHashtable.Add("PTCP_CEREBALID", CEREBAL_ID.Text);
                    vHashtable.Add("PTCP_CEREBAL_TRAIT", CEREBAL_TRAIT.Text);
                    vHashtable.Add("PTCP_OBSERVATION", Textarea2.InnerText);
                    vHashtable.Add("PTCP_STATUS", "1");
                    vHashtable.Add("TYPE", "INS");
                    DBManager.ExecInsUps(vHashtable, "INS_PT_CEREBAL_PALSY", vATSession);
                }
            }
